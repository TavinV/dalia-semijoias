...
const EditFornecedorModal = ({ fornecedor, isOpen, onClose, onSuccess }) => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!fornecedor?._id) return;
    setLoading(true);
    setError(null);
    try {
      const res = await api.put(`/fornecedores/${fornecedor._id}`, form);
      if (res.data.success) {
        onSuccess?.(res.data.data);
        onClose();
      } else {
        setError(res.data.message || "Erro ao atualizar fornecedor");
      }
    } catch (err) {
